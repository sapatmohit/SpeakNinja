...
//       body: Stack(
//         children: [
//           // Wave image at the top, positioned above the screen for cut-off effect
//           Positioned(
//             top: -80, // Move image up to create cut-off
//             left: 0,
//             right: 0,
//             child: Image.asset(
//               'assets/wave2.png',
...
class _RoadmapPageState extends State<RoadmapPage> {
  ...
  Future<void> _loadUsername() async {
    final prefs = await SharedPreferences.getInstance();
    final name = prefs.getString('username') ?? 'Learner';
    print("-------------------------------------------📥 Loaded username: $name");
    setState(() {
      _username = name;
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          // White background section with wave, welcome text, profile, and section button
          Container(
            height: 240,
            color: Colors.white,
            child: Stack(
          ...
          Expanded(
            child: Stack(
              children: [
                Container(
                  color: Colors.white, // Ensure list background is white
                ),
                Positioned.fill(
                  left: 16 + circleRadius,
                  child: IgnorePointer(
